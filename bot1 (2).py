"""
BOT 1 - SOURCE BOT
Fetches videos from source channels via RSS feed and uploads to main channel.
NO yt-dlp duration check (fails silently in GitHub Actions).
Uses feedparser only for video discovery.
"""
...
# ── Get videos from RSS (no yt-dlp, no silent failures) ───
...
def run_bot1():
    print("\n========================================")
    print("BOT 1 STARTED — Sourcing Videos")
    print("========================================\n")

    uploaded = load_json(UPLOADED_LOG, [])
    archive  = load_json(ARCHIVE_LOG, [])
    
    # Ensure uploaded is a list
    ...
    # Fallback to archive if nothing new found
    if count == 0:
        print("\n⚠️ No new videos found in source channels.")
        if archive:
            # Pick one that hasn't been re-uploaded recently
    ...
    print(f"\n========================================")
    print(f"BOT 1 DONE — Uploaded {count} video(s)")
    print(f"========================================\n")
